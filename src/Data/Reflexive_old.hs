...
-- cg t -> dg t
-- f (cg t) -> f (dg t)
-- OOh, this one is easy too: fmap lifts stuff and all is left is coercing between
-- f (g a) ~ (f :.: g) a

-- Comp1 { unComp1 :: f (g a) } :: (f :.: g) a
instance (Functor f, Recursive (g a), XMap1 g (Rec (g a)) a) => Recursive ((f :.: g) a) where
  type ForallRec ((f :.: g) a) t = ForallRec (g a) t
  ...
  push (Comp1 x) = undefined
    where
      x' :: a
      x' = undefined -- push <$> (unsafeCoerce x :: XMap (Rec (g a)) (g a))

  -- xmap1 :: XMap (Rec (f a)) (f a) -> f (XMap (Rec (f a)) a)



   -- ((:.:) f g (XMap (Rec ((:.:) f g a)) a)
-- Expected type: XMap (Rec ((:.:) f g a)) ((:.:) f g a)
-- Actual type: f (XMap (Rec ((:.:) f g a)) a)
   --
